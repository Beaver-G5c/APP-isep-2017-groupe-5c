<?php
	...
	function add_alert($description, $name, $id_target, $type_alerte, $niveau_alerte, $db,$id2){
	    ...
	    //echo ('on mapppelle');
         	//echo ("des : ".$description.' name '.$name.' id target '.$id_target.' type : '.$type_alerte.' niveau '.$niveau_alerte);
        // On prépare la requete d'ajout à la base de données
         	
         	
         	$req=$db->prepare("INSERT INTO notification(ID_user,notif_description,notif_date,notif_name,notif_statut,notif_type,notif_lvl,from_sav,ID_notif) VALUES(:ID_user,:notif_description,NOW(),:notif_name,:notif_statut,:notif_type,:notif_lvl,:from_sav,:ID_notif)");
		// On execute la requete en y ajoutant les paramètres rentrés lors de l'appel de la fonction.
         	$req->execute(array('ID_user'=>$id_target,'notif_description'=>$description,'notif_name'=>$name,'notif_statut'=>$st,'notif_type'=>$type_alerte,'notif_lvl'=>$niveau_alerte,'from_sav'=>$sav,'ID_notif'=>$id));
        //$req=$db->query("INSERT INTO notification(ID_user,notif_description,notif_date,notif_name,notif_statut,notif_type,notif_lvl,from_sav,ID_notif) VALUES('1','kiki',NOW(),'kiki','kiki','kiki','8','0','78')");
	}
